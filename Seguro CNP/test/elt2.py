'''Este script possui a mesma funcionalidade do etl.py; entretanto, 
não utiliza um endereço fixo para a planilha, recorrendo à biblioteca PyQt5 
para permitir a seleção da planilha por meio do explorador do Windows.'''
...
# --- USANDO PYQT5 PARA SELEÇÃO DA PLANILHA ---
...
logging.info(f"Arquivo selecionado: {excel_file}")
...
def exibir_dados_em_tabela(df: pd.DataFrame):
    ...
    print(tabulate(df_formatado, headers='keys', tablefmt='psql', showindex=False))
...
try:
    primeira_data = pd.to_datetime(df_005["logDatMov"].dropna().iloc[0], dayfirst=True)
    mes_map = {
    ...
    mes_abbr = primeira_data.strftime("%m")
    ano_abbr = primeira_data.strftime("%y")
...
# Exibir os dados antes de inserir no banco
...
# TESTE: Exibir os SQLs antes de executar
with engine.connect() as conn:
