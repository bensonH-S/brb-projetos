...
with engine.connect() as conn:
    for _, row in df_movimentacao.iterrows():
        ...
        """
        logging.info(f"Executando SQL: {sql_query}")  # Logando os SQLs antes de executar

# **Atualizar a coluna do mês atual na tabela cnp_historico**
with engine.begin() as conn:  # Garantir commit automático
    for _, row in df_movimentacao.iterrows():
        query_update = text(f"""
            UPDATE cnp_historico
        ...
        conn.execute(query_update, {
            "media_mes": round(float(row["Media_Mes"]), 2),  # Arredondando para 2 casas decimais
            "cnp": int(row["ptaCod"])
        })
